...
export const getScores = (
  ...
  return USE_SERVER
    ? callAPI<ProjectScore>(`/projects/${projectID}/scores`, token).catch(
        errorHandler,
      )
    : Promise.resolve(defaultScoreFetcher(projectID));
};
...
export const getTeam = (
  ...
  return USE_SERVER
    ? callAPI<ProjectTeam>(`/projects/${projectID}/team`, token, "POST").catch(
        errorHandler,
      )
    : Promise.reject("NO DEFAULT TEAM");
};
